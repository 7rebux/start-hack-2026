...
interface StudentDetailNodeProps {
  data: Student
}

export function StudentDetailNode({ data }: StudentDetailNodeProps) {
  return (
      ...
      {data.skills.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-2">
          {data.skills.map(skill => (
            <span key={skill} className="text-xs px-2 py-0.5 rounded-full bg-sky-50 text-sky-700 border border-sky-200">
              {skill}
            </span>
          ))}
        </div>
