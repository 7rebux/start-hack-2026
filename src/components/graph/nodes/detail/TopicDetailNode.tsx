...
interface TopicDetailNodeProps {
  data: Topic & { fieldNames: string[] }
}

export function TopicDetailNode({ data }: TopicDetailNodeProps) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl shadow-lg p-5 w-[420px]">
      ...
      <div className="flex flex-wrap gap-1.5 mb-3">
        {data.degrees.map(d => (
          <span key={d} className="text-xs px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-700 font-medium">
            {degreeLabel[d]}
          </span>
        ))}
      </div>
